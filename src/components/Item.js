...
const Duration = ({ item, theme }) => {
  ...
  return (<div className="wrap">
    <span className="duration" style={{ color: theme === 'Vibrant' && 'white' }}>{formattedDate}</span>
    {item.data.children.length === 0 && <span className="duration" style={{color: theme === 'Vibrant' && 'white'}}> | </span>}
    {item.data.children.length === 0 && <span className="duration" style={{ color: theme === 'Vibrant' && 'white' }}>{formatTime(item.duration)}</span>}
  </div>)
}
        ...
        <div className="text-wrapper">
          <div className="center-align">
            ...
            <div className="v-wrapper">
              <span className={`label ${item.data.children.length > 0 && 'parent-label'}`} style={{ fontSize: profile.prefs.appearence.style === 'Compact' && '1.2rem', color: profile.prefs.appearence.style === 'Vibrant' && 'white'}}>{item.label}</span>
