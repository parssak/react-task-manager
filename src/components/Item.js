...
const Duration = ({ item, theme }) => {
  let formattedDate = "Someday idk";
  let diff = daysIntoYear(new Date(item.data.date)) - daysIntoYear(new Date());
  if (diff === 0)
    formattedDate = "Today"
  else if (diff === 1)
...
const Content = ({ item, completeItem, profile, setCollapse, collapse, hovering }) => {
  ...
  return (
    <div className="grid-container">
      <div className="completed-area">
        <button onClick={e => toggleComplete(e)} style={{ backgroundColor: item.completed && 'rgb(78, 194, 117)' }} />
      </div>
      <div className="title-area">
        <span className={`label ${item.data.children.length > 0 && 'parent-label'}`}
          style={{ fontSize: profile.prefs.appearence.style === 'Compact' && '1.2rem', color: (profile.prefs.appearence.style === 'Vibrant' || profile.prefs.appearence.style === 'Compact') && 'white' }}>{item.label}
        </span>
      </div>
...
const Item = ({ item, min, max, updated, selectItem, selectedItem, className }) => {
  ...
  const drop = e => {
    ...
    const oldParent = profile.items.filter(a => a.key === card_id)[0].data.parent;
  ...
  let numInnerComplete = 0;
  item.data.children.forEach(e => profile.items.filter(a => a.key === e)[0].completed && numInnerComplete++);
  return (
    <div className={`item glassy-inner ${className}`} id={item.key}
        style={{
        ...
        onDrop={e => drop(e)}
      >
        <Content item={item} completeItem={completeItem} profile={profile} collapse={collapse} setCollapse={setCollapse} hovering={ hovering}/>
        {!collapse &&
